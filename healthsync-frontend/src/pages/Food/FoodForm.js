...
const FoodForm = () => {
  ...
  const [meals, setMeals] = useState([
    { name: "", calories: 0, protein: "", carbs: "", fats: "" },
  ]);
  ...
  const calculateCalories = (protein, carbs, fats) => {
    ...
    return (
      parseFloat(protein) * 4 + parseFloat(carbs) * 4 + parseFloat(fats) * 9
    );
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      ...
      <div className="max-w-xl mx-auto mt-10">
        ...
        >
          ...
          {meals.map((meal, index) => (
            <div key={index} className="mb-4 bg-gray-100 p-3 rounded">
              ...
              <input
                ...
                required
                className="mt-2 p-2 w-full border-gray-300 rounded-md"
              />
              <input
                type="text"
                readOnly
                value={`Calories: ${meal.calories.toFixed(0)}`}
                className="mt-2 p-2 w-full border-gray-300 rounded-md bg-gray-200 text-gray-600"
              />
              <div className="grid grid-cols-3 gap-4">
                {["protein", "carbs", "fats"].map((nutrient, idx) => (
                  <input
                    ...
                    value={meal[nutrient]}
                    onChange={(e) =>
                      handleMealChange(index, nutrient, e.target.value)
                    }
                    className="mt-2 p-2 border-gray-300 rounded-md"
                  />
                ))}
              </div>
              {meals.length > 1 && (
          ...
          <button
            type="submit"
            className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded block mt-4 w-full"
          >
            Submit
          </button>
        </form>

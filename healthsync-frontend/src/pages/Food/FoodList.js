...
const FoodList = () => {
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      await axios.put(
        `http://127.0.0.1:5000/api/meals/${editMealId}`,
        updateData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
                ...
                {editMealId === meal._id.$oid ? (
                  <form onSubmit={handleUpdate} style={styles.form}>
                    ...
                    <div style={styles.formGroup}>
                      ...
                      <input
                        type="number"
                        value={editFormData.calories.toFixed(2)}
                        readOnly
                        style={styles.input}
                      />
                    </div>
                    ...
                    <div style={styles.formGroup}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("fats", e.target.value)
                        }
                        style={styles.input}
                      />
                    </div>
                  ...
                  <>
                    <p style={styles.text}>
                      <strong>ID:</strong> {meal._id.$oid}
                    </p>
                    <p style={styles.text}>
                      <strong>User ID:</strong> {meal.user.$oid}
                    </p>
                    <p style={styles.text}>
                      <strong>Date:</strong>{" "}
                      {new Date(meal.date.$date).toLocaleDateString()}
                    </p>
                    <p style={styles.text}>
                      <strong>Created At:</strong>{" "}
                      {new Date(meal.created_at.$date).toLocaleString()}
                    </p>
                    {meal.meals.map((subMeal, index) => (
                      <div key={index} style={styles.subMeal}>
                        <p style={styles.text}>
                          <strong>Name:</strong> {subMeal.name}
                        </p>
                        <p style={styles.text}>
                          <strong>Calories:</strong>{" "}
                          {subMeal.calories.toFixed(2)}
                        </p>
                        <p style={styles.text}>
                          <strong>Protein (g):</strong>{" "}
                          {subMeal.protein.toFixed(2)}
                        </p>
                        <p style={styles.text}>
                          <strong>Carbs (g):</strong> {subMeal.carbs.toFixed(2)}
                        </p>
                        <p style={styles.text}>
                          <strong>Fats (g):</strong> {subMeal.fats.toFixed(2)}
                        </p>
                      </div>
                    ))}
                    <div style={styles.buttonGroup}>
                      ...
                      <button
                        onClick={() => handleDelete(meal._id.$oid)}
                        style={styles.button}
                      >
                        Delete
                      </button>
